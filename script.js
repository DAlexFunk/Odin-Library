...
function checkValidityOfWholeForm() {
  if (!title.validity.valid) {
    return false;
  } else if (!author.validity.valid) {
    return false;
  } else if (pageCount.validity.rangeUnderflow) {
    return false;
  }
  return true;
}

function actionOnPress(evt) {
  evt.preventDefault();
  if (!checkValidityOfWholeForm()) {
    return;
  }
...
const pageCount = document.querySelector("input#pageCount");
